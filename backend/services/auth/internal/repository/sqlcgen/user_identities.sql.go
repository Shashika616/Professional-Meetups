...
SELECT id, user_id, provider, subject, email, linked_at FROM user_identities WHERE provider = $1 AND subject = $2
`
...
func (q *Queries) InsertIdentity(ctx context.Context, arg InsertIdentityParams) (UserIdentity, error) {
...
SELECT id, user_id, provider, subject, email, linked_at FROM user_identities WHERE user_id = $1 ORDER BY linked_at
`
