...
- **[Action Tracker](00-project/action-tracker.md)** — the current checklist: what needs Shashika's action, what's blocking Claude Code, what's a deliberately-deferred known gap. Check this before assuming anything is done or still undecided — it's kept current every session, unlike the historical log in [Project State](00-project/project-state.md).
...
- **04 - Decisions** — [ADR-001](04-decisions/adr-001-verified-real-world-professional-networking.md) through [ADR-012](04-decisions/adr-012-level-2-3-verification-delivery-and-identity-anchors.md). Why we chose what we chose, so nobody re-litigates it three weeks later.
